          ...
          definition = function(x, sF){
              ...
              for (v in 1:nrow(imgTypeDF)) {
                  for (i in 1:nrow(subDirs)) {
                      ...
                      if (length(sF@imageNums) != 0) {
                          imgInfoDFList[[iterTrack]] <- imgInfoDFList[[iterTrack]] %>%
                              dplyr::filter(imageNums %in% sF@imageNums)
                      }
                      if (nrow(imgInfoDFList[[iterTrack]]) == 0) {
                          stop("No images found after 'imageNums' filter applied for image type: ",
                               imageTypes(sF)[v])
                      }
...
.imgInfo <- function(imageFile) {
    ...
    if (grepl("ima", imageFileSplit[1])) {
        imgInfoDF$imageNums <- as.numeric(gsub("ima", "", imageFileSplit[1]))
        ...
        imgInfoDF$imageNums <- as.numeric(imageFileSplit[2])
