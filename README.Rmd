...
```{r get files example}
#define the simulation directory. For this example it is relative and is one simulation.
#if you install the package using devtools::install_github then these files will not be available.
...
ggplot(simData@data) + 
    ...
    theme(aspect.ratio = 1)
```
That's a lot of data! It is important to set the "SimulationFilter" to match what data you want so that this doesn't happen. Further the process can use a lot of memory when many large files are loaded. Therefore, try to load in only the files you want and limit the amount of data filtering.
