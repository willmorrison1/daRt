...
```
It's important that `SimulationFilter` matches only the data you actually want.
...
### Memory management
This section demonstrates memory management when performing analysis on a relatively large set of files. `getData()` loads all data to memory which is problematic when loading many large files (e.g. Radiative Budget). It is assumed that the user will perform some analysis on subsets of the raw data in a way that reduces the overall size of the data. Files here are loaded in two ways: option 1 uses the default `getData()` to load and then analyse all data at once. Option 2 loads and analyses the data in pieces, which has a much smaller memory footprint (but may be slower). Both options give the same result.
