...
Under windows especially, there are some issues with paralellisation. When running `getData()` you may get "invalid connection" error. In this case you will always need to run with nCores > 1 e.g.  `getData(..., nCores = 2)`.
